...
<span>MEMBERSHIP ID: </span><input type="text" style="width:265px; height:30px;"  value="MC20-<?php $prefix= md5(time()*rand(1, 2)); echo strip_tags(substr($prefix ,0,4));?>" name="membership_id" readonly Required /><br> <i><b>Make Sure You Remember your Membership ID</b></i> <br>
<span>Password : </span><input type="password" style="width:265px; height:30px;" id="pswd2"   name="password1" Required /><br>
<input type="checkbox" onclick="Shossword()" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters">Show Password <br>
<script>
    function Shossword() {
...
<div >
<button class="btn btn-success btn-block btn-large" style="width:267px;"><i class="icon icon-save icon-large"  onclick="validateForm()"></i>Register</button> <br>
